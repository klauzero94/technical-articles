...
- **Análise:** O(k) - linear com número de namespaces únicos
...
- Seja C = (namespace, tipo_erro) o contexto de execução
- Seja φ(C) a função de normalização que extrai a parte final do namespace
- Seja h(φ(C)) a função hash aplicada ao contexto normalizado
- O identificador I = "PREFIX-TYPE-" + h(φ(C))
